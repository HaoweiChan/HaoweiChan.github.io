...
金額全部馬賽克掉了，剩下的才是這篇文章真正想講的東西：曲線的形狀、回撤的深度、配置的比重。2025 年 4 月那道 −53.6% 的回撤是真的虧損（回撤走的是 TWR，已經把入金出金剝掉了）；2026 年 6 月中那條綠色帶狀的躍升則是新帳戶入金，不是報酬。**這兩件事在圖上長得幾乎一樣，唯一能分辨的方法是把現金流獨立記錄下來**——這也是為什麼下一節要花那麼多篇幅講 T+2 與交割款。
...
## 實例：交割款如何偽造出一次 −6.6% 的假崩盤

前面 T+2 那一段講的是機制，但機制我是**在它騙過我之後才補上的**。這裡把那天的數字攤開，因為它示範了這整套系統最危險的失效模式。

2026 年 7 月 9 日，現股帳戶做了一次換股：賣掉兩檔、減碼一檔、買進兩檔，賣超約當帳戶淨值的 6.9%。當晚排程跑完，Discord 收到的訊息是 **−6.61%**。

問題是那天帳戶其實幾乎沒動。當時的算式是：

```
NAV = 已結算現金 (acc_balance) + 持股市值
```

看起來很合理，錯在 `acc_balance` 是**已結算現金**。賣出的股票在成交當下就從市值消失了，但賣股所得要到 T+2 才會匯進交割帳戶。那筆錢在 7 月 9 日晚上既不在「市值」裡，也不在「現金」裡——它在半空中，而算式沒有一格能裝它。

向券商查 `settlements()` 才看到它躺在那：`T=2, 交割日 7/13`。加回去：

| 算式 | 已結算現金 | 持股市值 | 未交割款 (T+1/T+2) | 對前日變動 |
|---|:---:|:---:|:---:|---|
| 原本（錯） | ✓ | ✓ | ✗ | **−6.61%** |
| 修正後 | ✓ | ✓ | ✓ | **+0.25%** |

修正後的算式多了一項，正負號跟著交易方向走：賣出是「應收」為正，買進是「應付」為負，所以買進日的假暴增也一併修掉了。

```
NAV = 已結算現金 + Σ 未交割款 + 持股市值
```

這個修正之所以成立，是因為它在整個週期上自洽：等交割款入帳，未交割款歸零、現金增加同一筆數字，NAV 從頭到尾平滑。

真正值得記下來的是**它為什麼難抓**。一個 −6.61% 不會觸發任何警報——市場真的會跌 6%，前面那個「合理性邊界檢查」只擋得住版型解析錯亂級別的離譜數字，擋不住一個看起來完全像真的的數字。它沒有 crash，沒有寫 log，沒有紅字，就只是安靜地說了一個謊。**在資料工程裡，會 raise 的錯誤是最便宜的錯誤**；貴的是那種長得跟正確答案一模一樣的。

（順帶：用 API 清算價值 (liquidation value) 的海外券商帳戶完全沒這個問題——那個數字本來就含未交割部位。這個坑只屬於「自己把 NAV 加出來」的帳戶。）
