...
這條管道每天 23:00 自動吐出的成果長這樣——三家券商、五個帳戶，權益曲線、回撤與配置比重全部由前面那些對帳單 PDF 與 API 回應堆疊而成：

![多帳戶權益追蹤儀表板：上方是五個帳戶的堆疊權益曲線，左下是回撤百分比，右側是當日配置圓環圖。所有金額數字已打上馬賽克。](../../../assets/blog/portfolio-equity-dashboard.png)

金額全部馬賽克掉了，剩下的才是這篇文章真正想講的東西：曲線的形狀、回撤的深度、配置的比重。2025 年 4 月那道 −53.6% 的回撤是真的虧損（回撤走的是 TWR，已經把入金出金剝掉了）；2026 年 6 月中那條綠色帶狀的躍升則是新帳戶入金 NT$2.77M，不是報酬。**這兩件事在圖上長得幾乎一樣，唯一能分辨的方法是把現金流獨立記錄下來**——這也是為什麼下一節要花那麼多篇幅講 T+2 與交割款。
