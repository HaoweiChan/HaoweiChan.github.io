...
description: '一個常駐、會自己開口的個人助理是怎麼蓋起來的：六個讓它不像聊天機器人的特性，以及它變得可信的那一天。'
lang: 'zh-tw'
...
物理上很無聊：一台放在家裡、不關機的小電腦，上面跑兩個東西 —— 一個連到聊天軟體的訊息閘道，和一個排程器。旁邊有一個純文字的筆記庫（Markdown 檔案 + git），那是它的長期記憶。
...
我沒有做 app，沒有做 dashboard，沒有做網頁。它就在我本來每天都開著的聊天軟體裡講話。

這是它活過第二週的真正原因。

任何需要我「特地去打開」的介面，最後都會變成我不去打開的介面。而聊天軟體我本來就開著，訊息會推播到手機，歷史紀錄天生就有，多人多裝置同步不用我處理。我等於是免費拿到了通知系統、收件匣和存檔。
...
聊天軟體的頻道被我拿來當成整套系統的路由表。

一個頻道 = 一個角色 = 一份 prompt = 一個歸檔目的地。問市場的、問研究的、問技術的、看報表的、看警報的，各自有自己的頻道，各自有自己的人設與資料權限，產出的檔案也各自落在筆記庫的不同資料夾。
...
助理的每一項能力，實體上就是一個資料夾：一份說明它幹嘛、什麼時候該用的規格檔，加上幾支腳本。
...
它學到、產出、記下的所有東西，最後都是 Markdown 檔案，躺在一個 git 倉庫裡，我可以用一般的筆記軟體打開、可以 grep、可以直接手改。
...
**框架給我的**：訊息閘道、排程器、能力資料夾的載入機制、可選的 agent 模式。這些是現成的，設定一下就能跑，是簡單的那一半。

**我得自己設計的**：純文字的記憶層與它的欄位契約、通知的紀律（什麼時候該閉嘴）、頻道的職責切分、行為契約與判例庫、以及把想法變成工程項目的那條管線。

換句話說，runtime 幾乎不是問題。難的是那些沒有人幫你決定的東西 —— 它什麼時候可以打斷我、它記住的東西放在哪、它做決定時依據什麼。這些沒有一項是「接上模型」能解決的。
